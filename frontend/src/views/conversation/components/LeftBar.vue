<template>
  <div class="md:w-1/5 md:min-w-50 w-full px-4 box-border mb-4 lt-md:h-40 md:flex-grow overflow-hidden flex flex-col space-y-4">
    ...
    <div class="flex-grow flex flex-col">
      ...
      <n-button secondary strong type="primary" @click="emits('new-conversation')" :disabled="props.loading">
        <template #icon>
          <n-icon class="">
            <Add />
          </n-icon>
      ...
      <n-scrollbar class="h-0 flex-grow mt-4">
        <n-menu :content-style="{ backgroundColor: 'red' }" ref="menuRef" :disabled="props.loading" :options="menuOptions" :root-indent="18"
          v-model:value="convId"></n-menu>
      </n-scrollbar>
    </div>
  </div>
</template>
    ...
    onPositiveClick: () => {
      ...
      return new Promise((resolve) => {
        conversationStore.deleteConversation(conversation_id).then(() => {
          ...
          if (convId.value == conversation_id)
            convId.value = null;
        }).catch(() => {
